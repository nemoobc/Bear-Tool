...
test('E2E-probe: every $(\'#id\') reference across ALL js files exists in index.html', async () => {
  ...
  const dynamicSkip = new Set([
    ...
    // Add-network picker + "More" sheet + token autodetect, all injected at
    // runtime. Asserted to be generated by the test below, so this list cannot
    // quietly excuse an id that nothing ever creates.
    'mobileMoreBtn','cnForm','cnIcon','cnNameLabel','cnChainLabel','cnSearch',
...
test('E2E-probe: runtime-injected ids are really created (dynamicSkip is not a blind pass)', async () => {
  ...
  // user opens the mobile bar, the Add-network picker, the More sheet or the
  // Add-token modal. The ID probe skips them because a static grep cannot see
  ...
  assert.match(app, /function showMoreSheet\(/, 'showMoreSheet must exist');
